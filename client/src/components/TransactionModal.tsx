...
export default function TransactionModal({
  ...
  const currentBalance = useSelector(
    (state: AuthState) => state.authReducer?.authData?.user?.balance
  );
        ...
        sellStockMutation({ variables: { ticker, shares } })
          .then(({ data }) => {
            setTimeout(() => {
              ...
              dispatch({
                type: UPDATE_STOCK,
                payload: { ticker, stock: data?.sellStock.ownedStock },
              });
              dispatch({
                type: UPDATE_BALANCE,
                payload: { newBalance: data?.sellStock.newBalance },
              });
              setIsLoading(false);
              completeTransaction();
            }, 1500);
        ...
        buyStockMutation({ variables: { ticker, shares } })
          .then(({ data }) => {
            setTimeout(() => {
              ...
              dispatch({
                type: UPDATE_STOCK,
                payload: { ticker, stock: data?.buyStock.ownedStock },
              });
              dispatch({
                type: UPDATE_BALANCE,
                payload: { newBalance: data?.buyStock.newBalance },
              });
              setIsLoading(false);
              completeTransaction();
            }, 1500);
        ...
        <div className="relative bg-white rounded-lg shadow-xl mb-16 dark:bg-darkField">
          ...
          {isComplete ? (
            <div className="h-full p-5 py-16">
              ...
              <h5 className="text-gray-900 dark:text-gray-200 text-md text-center">
                {completionDetails}
              </h5>
            </div>
          ) : (
              ...
              >
                ...
                <div>
                  <label className="flex relative items-center select-none">
                    <span className="text-sm font-medium text-gray-900 dark:text-gray-200 mr-3">
                      Shares{" "}
                    </span>
                ...
                {errors && (
                  <div className="flex justify-center">
                    <div className="flex items-center ml-1 mb-2 select-none">
                      <label className="text-xs text-red-600 dark:text-red-500">
                        {errors}
                      </label>
                    </div>
                  </div>
