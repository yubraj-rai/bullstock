...
const ForgotPassword: React.FC = () => {
  ...
  const handleSendOtp = async () => {
    ...
    await sendOtp({ variables: { username } })
      .then(({ data }) => {
        setErrors('');
        console.log(data); // You can use data here if needed
    ...
    <div className='dark:bg-darkBg h-screen w-full flex flex-auto items-center justify-center'>
      <div className='w-full max-w-sm mx-auto overflow-hidden bg-white dark:bg-darkField text-gray-700 dark:text-gray-200 rounded-2xl shadow-xl'>
        <div className='p-8 relative'>
          ...
          <h2 className='text-2xl font-bold text-center text-gray-700 dark:text-white'>Trouble logging in?</h2>
          <p className='mt-2 text-sm text-center text-gray-600 dark:text-gray-300'>
            Enter your email and we will send you a OTP to get back into your account.
          </p>
