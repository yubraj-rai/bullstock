...
import { userBalanceVar } from '../cache/cacheConfig';
...
const AccountPage = () => {
    ...
    // const balance = useReactiveVar(userBalanceVar);
        ...
        if (transferAmount > 0) {
            ...
            depositMutation({ variables: { amount: transferAmount } })
                .then(({ data }: any) => {
                    setTimeout(() => {
                        setErrors('');
                        userBalanceVar(data?.deposit?.newBalance);  // Update balance in the cache
        ...
        if (transferAmount > 0) {
            ...
            withdrawMutation({ variables: { amount: transferAmount } })
                .then(({ data }) => {
                    setTimeout(() => {
                        setErrors('');
                        userBalanceVar(data?.withdraw?.newBalance);  // Update balance in the cache
                                                ...
                                                <form
                                                    ...
                                                    <div className='flex mt-6'>
                                                        <button
                                                            ...
                                                            className="w-full flex justify-center px-6 py-2 leading-5 text-white transition-colors duration-200 transform bg-[#6dbfdd] rounded-md hover:bg-[#5ba3c2] focus:bg-[#5ba3c2] focus:outline-none"
>
