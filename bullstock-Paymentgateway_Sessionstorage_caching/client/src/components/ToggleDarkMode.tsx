...
export default function ToggleDarkMode() {
    ...
    // Assign colors from your monochromatic palette
    const sunColor = "#246aaf";  // Color for sun
    const moonColor = "#246aaf"; // Updated color for moon
        ...
        <Switch className={'group'} checked={isDark} onChange={setIsDark}>
            <span className={(isDark ? 'bg-gray-700 hover:text-gray-100' : ' bg-gray-100 ') + ' rounded-full shadow-md p-1 h-8 w-16 flex'}>
                <span
                    ...
                    {isDark ? (
                        <svg xmlns='http://www.w3.org/2000/svg' className='h-full w-full' viewBox='0 0 20 20' fill={moonColor}>
                            <path d='M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z' />
                        </svg>
                    ) : (
                        <svg xmlns='http://www.w3.org/2000/svg' className='h-full w-full' viewBox='0 0 20 20' fill={sunColor}>
                            <path
                                fillRule='evenodd'
